...
const CHALLENGE_QUERY_KEYS = {
  ...
  GROUP: {
    ...
    // 목록
    LIST: (category: ChallengeCategoryType, input: string | undefined) => `[challenges, group, ${category}, ${input}]`,
    // 인증 내역 목록 조회
...
const MEMBER_QUERY_KEYS = {
  ...
  CHALLENGE: {
    GROUP: {
      ...
      COUNT: '[member, challenges, group, participations, count]', // 참여한 단체 챌린지 카운트 조회

      // 챌린지 인증 내역
      VERIFICATIONS: (challengeId: number) => ({
        path: `[member, challenges, group, participations, ${challengeId}, verficiations]`,
      }),
    },
  },
}
